...
class MCCHO(nn.Module):
    ...
    def __init__(self,
                 ...
                 rgb_weight=1.0, occupancy_weight=1.0,
                 segmentation_weight=1.0, consistency_weight=0.0, args=None):
        super().__init__()
        ...
        # Add extra output for background (0), hand (1), or object (2) label
        if self.args.segmentation_label:
            self.decoder_label = nn.Linear(decoder_embed_dim, 3, bias=True)
            self.loss_segmentation = nn.CrossEntropyLoss()
            self.loss_consistency = nn.BCEWithLogitsLoss()
    ...
    def forward_decoder(self, x, unseen_xyz):
        ...
        if self.args.segmentation_label:
            pred_label = self.decoder_label(x)
            pred_label = pred_label[:, -unseen_xyz.shape[1]:, :]
            return pred, pred_label

        return pred

    def forward_loss(self, pred, unseen_occupy, unseen_rgb,
                     pred_label=None, unseen_seg=None):
        ...
        if self.args.segmentation_label:
            pred_seg = pred_label[:, :, :].reshape((-1, 3))
            gt_seg = unseen_seg[..., None].long().reshape((-1,))
            segmentation_loss = self.loss_segmentation(pred_seg, gt_seg) * self.segmentation_weight
            # TODO(jwu): Remove this or fix.
            '''
            # Encourage segmentation to be consistent with occupancy
            pred_seg_foreground = torch.clip(pred_seg.max(dim=-1)[1], 0., 1.)
            consistency_loss = self.loss_consistency(
                pred[:, :, :1].reshape((-1, 1)),
                pred_seg_foreground.reshape((-1, 1)).float()
    ...
    def forward(self, seen_images, seen_xyz, unseen_xyz, unseen_rgb, unseen_occupy, valid_seen_xyz,
                unseen_seg=None, cache_enc=False):
        """Model forward pass.

        Args:
            seen_images: [B, 3, 800, 800] tensor of input RGB images.
            *seen_xyz: [B, 112, 112, 3] tensor of input point cloud from depth.
            unseen_xyz: [B, 550, 3] tensor of uniform sampling of query points.
            unseen_rgb: [B, 550, 3] tensor of RGB values copied from closest
                ground truth point. (zeros at test time)
            unseen_occupy: [B, 550] tensor of 'labels', e.g. for each query,
                whether it is within a threshold of a ground truth point.
                (zeros at test time)
            valid_seen_xyz: [B, 112, 112] bool tensor indicating whether each
                seen_xyz point is valid (finite) or not.
            unseen_seg: [B, 500] tensor of hand (0), object (1), or
                background (-1) labels, if self.args.segmentation_label True.
        Returns:
            loss: Training loss.
            pred: [B, 550, 769] tensor of predicted point cloud, where
                pred[..., 0] stores occupancy and pred[..., 1:] stores
                color (256x3=768). If args.regress_color, shape is [B, 550, 4].
                If args.segmentation_label, shape is [B, 550, K + 3] where K is
                either 769 or 4.
        """
        ...
        # Distinguish between hand and object geometry
        if self.args.segmentation_label:
            pred, pred_label = self.forward_decoder(latent, unseen_xyz)
            loss = self.forward_loss(pred, unseen_occupy, unseen_rgb, pred_label, unseen_seg)
            # Concatenate outputs
            pred = torch.cat([pred, pred_label], axis=-1)
        else:
