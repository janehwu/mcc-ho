...
def get_all_dataset_maps(co3d_path, dataset_cache, holdout_categories, run_viz):
    ...
    for category in all_categories:
        ...
        if run_viz:
            dataset_map['test'] = torch.load(f'{dataset_cache}/{category}_test.pt')
            if not holdout_categories or category in HOLDOUT_CATEGORIES:
                test_dataset_maps[category] = dataset_map['test']

    print('Loaded', len(train_dataset_maps), 'categories for train')
    print('Loaded', len(val_dataset_maps), 'categories for val')
    print('Loaded', len(test_dataset_maps), 'categories for test')
    return train_dataset_maps, val_dataset_maps, test_dataset_maps
