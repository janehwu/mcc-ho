...
import sys
sys.path.append('third-party/hamer')
...
def load_hamer(ckpt_dir):
    '''Adapted from hamer.models.load_hamer.'''
    model_cfg = get_config(os.path.join(ckpt_dir, 'model_config.yaml'), update_cachedir=True)
    ...
    checkpoint_path = os.path.join(ckpt_dir, 'checkpoints/hamer.ckpt')
...
def run_demo(
    img_path,
    out_folder,
    scaled_focal_length=1000,
    ckpt_dir='./_DATA/hamer_ckpts',
    side_view=False,
    full_frame=True,
    save_mesh=True,
    rescale_factor=2.0,
    ...
    model, model_cfg = load_hamer(ckpt_dir)

    # Setup HaMeR model
    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
    model = model.to(device)
    model.eval()
    print('Done loading HaMeR model')
